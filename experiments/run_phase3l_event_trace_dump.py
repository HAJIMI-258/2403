"""Dump Phase 3L lineage-bound event traces and continuation lifecycle rows."""
...
from experiments.phase3x_utils import build_phase3x_event_trace, collect_phase3x_audit_rows


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Dump Phase 3L event trace rows.")
    ...
    parser.add_argument("--tracking-scan", default="")
...
def main() -> None:
    ...
    memory_override = _resolve_memory_override(args.tracking_scan)
...
def _resolve_memory_override(scan_path: str) -> dict[str, object] | None:
    if not scan_path:
        return None
    rows = load_csv_rows(scan_path)
    if not rows:
        return None
    explicit = [row for row in rows if str(row.get("is_best", "0")).lower() in {"1", "true", "yes"}]
    best_row = explicit[0] if explicit else pick_best_scan_row(rows, load_phase3x_before_lookup())
    return {
